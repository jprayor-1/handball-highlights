...
def run_video_processing(key: str) -> dict:
    ...
    with tempfile.NamedTemporaryFile(delete=True, suffix=ext) as temp_video:
        ...
        try:
            ...
            clipped_highlights = clip_and_upload_pipelined(video_path, top_20_highlights)
        finally:
            if video_path != temp_video.name and os.path.exists(video_path):
                os.remove(video_path)

    logger.info({"event": "clipped_highlights", "key": key, "count": len(clipped_highlights)})
